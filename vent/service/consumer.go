...
func (c *Consumer) Run(stream bool) error {
	...
	go func() {
		...
		evs, err := cli.GetBlocks(context.Background(), request)
		if err != nil {
			doneCh <- errors.Wrapf(err, "Error connecting to events stream")
			return
		}
			...
			if err != nil {
				if err == io.EOF {
					...
					if c.Closing {
						...
						doneCh <- errors.Wrapf(err, "Error receiving events")
						return
					}
				}
			}
